...
const sources = computed(() => ['全部', ...new Set(sampleArticles.map((a) => a.source))])
const tags = computed(() => ['全部', ...new Set(sampleArticles.flatMap((a) => a.tags))])

const filteredArticles = computed(() =>
  [...sampleArticles]
    .sort((a, b) => Number(new Date(b.publishedAt)) - Number(new Date(a.publishedAt)))
    .filter((item) => {
      const matchSource = selectedSource.value === '全部' || item.source === selectedSource.value
      ...
      >
        <div class="flex flex-col gap-2">
          ...
          <div class="flex flex-wrap gap-2">
            <UBadge v-for="tag in item.tags" :key="tag" color="gray" variant="soft">
              {{ tag }}
            </UBadge>
            <UBadge v-if="item.featured" color="primary" variant="soft" icon="i-heroicons-sparkles">
              精选
            </UBadge>
          </div>
        </div>
