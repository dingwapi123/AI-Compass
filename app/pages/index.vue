  ...
  <div class="space-y-12 text-slate-900 dark:text-slate-50 transition-colors">
    <section class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] items-center">
      <div class="space-y-6">
        ...
        <h1
          class="text-3xl sm:text-4xl lg:text-5xl font-semibold leading-tight text-slate-900 dark:text-white"
        >
          聚合最新 AI 动态，<br />
          一键直达产品官网
        </h1>
        <p class="text-lg text-slate-600 dark:text-slate-200 leading-relaxed">
          收录主流 AI
          资讯源，分类导航热门产品（编码类、图像生成、视频生成），为你节省信息筛选和跳转时间。
        </p>
        ...
        >
          <span class="inline-flex items-center gap-1">
            <UIcon
              ...
              class="h-4 w-4 text-sky-500 dark:text-sky-300"
            />
            实时筛选与搜索
          </span>
          <span class="inline-flex items-center gap-1">
      ...
      <div class="relative">
        <div
          class="absolute -inset-6 rounded-3xl bg-gradient-to-br from-sky-100 via-white to-transparent blur-2xl dark:from-slate-800 dark:via-slate-900 dark:to-transparent"
        />
        <div
          class="relative rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900/80"
        >
          <div
          ...
          >
            ...
            <NuxtLink
              to="/news"
              class="text-sm text-sky-600 hover:text-sky-700 dark:text-sky-300 dark:hover:text-sky-200"
              >更多</NuxtLink
            >
          </div>
          <div class="divide-y divide-slate-200 dark:divide-slate-800">
            <div v-for="item in featuredArticles" :key="item.id" class="p-6">
              ...
              <p
                class="mt-2 text-sm text-slate-600 dark:text-slate-300 line-clamp-2"
              >
                {{ item.summary }}
              </p>
