...
<template>
  <div class="space-y-8 text-slate-900 dark:text-slate-50 transition-colors">
    <div class="space-y-2">
      <h1 class="text-3xl font-semibold text-slate-900 dark:text-white">AI 产品导航</h1>
      <p class="text-slate-600 dark:text-slate-300">分类、标签与定价过滤，卡片直达官网。</p>
    </div>
    ...
    <div class="flex flex-wrap gap-3 items-center">
      ...
      />
      <USelect v-model="selectedCategory" :options="filtersCategory" icon="i-heroicons-rectangle-stack" color="primary" />
      <USelect v-model="selectedPricing" :options="filtersPricing" icon="i-heroicons-currency-dollar" color="info" />
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-2">
      ...
      >
        <div class="flex flex-col gap-2">
          ...
          <div v-if="product.updatedAt" class="text-xs text-slate-500 dark:text-slate-400">
            最近更新：{{ new Date(product.updatedAt).toLocaleDateString() }}
          </div>
          <div class="pt-2">
            <UButton :href="product.url" target="_blank" color="info" icon="i-heroicons-arrow-top-right-on-square">
              访问官网
            </UButton>
          </div>
        </div>
